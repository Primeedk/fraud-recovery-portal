document.addEventListener('DOMContentLoaded', () => {
  ...
  function filterAndRenderTable() {
    ...
    filtered.forEach(sub => {
      ...
      row.innerHTML = `
        ...
        <td style="text-align: center; white-space: nowrap;">
          ...
          <button type="button" class="btn btn-danger btn-sm btn-delete-case" data-ref="${sub.case_ref}" style="min-height:34px; padding: 0 0.5rem; margin-left: 0.25rem;" aria-label="Delete case">
    ...
    // Attach Delete Case listeners
    tableBody.querySelectorAll('.btn-delete-case').forEach(btn => {
      btn.addEventListener('click', () => {
        const caseRef = btn.getAttribute('data-ref');
        executeDeleteCase(caseRef);
      });
    });
  }
  ...
  // ==========================================
  // Delete Case Submission
  // ==========================================
  async function executeDeleteCase(caseRef) {
    if (!caseRef) return;
    
    const confirmDelete = confirm(`Are you sure you want to permanently delete Case ${caseRef}? This action cannot be undone.`);
    if (!confirmDelete) return;

    try {
      if (supabaseClient) {
        // Delete from Supabase
        console.log(`Syncing deletion to Supabase for ${caseRef}...`);
        const { error } = await supabaseClient
          .from('frip_submissions')
          .delete()
          .eq('case_ref', caseRef);
        ...
        showToast(`Case ${caseRef} successfully deleted from Cloud.`);
      } else {
        // Delete from LocalStorage
        const localDataStr = localStorage.getItem(LOCAL_SUBMISSIONS_KEY);
        if (localDataStr) {
          let localSubmissions = JSON.parse(localDataStr);
          localSubmissions = localSubmissions.filter(s => s.case_ref !== caseRef);
          localStorage.setItem(LOCAL_SUBMISSIONS_KEY, JSON.stringify(localSubmissions));
          showToast(`Case ${caseRef} deleted locally.`);
        }
      }
      
      // Update local memory state
      submissions = submissions.filter(s => s.case_ref !== caseRef);
      
      // Update UI elements
      calculateAnalytics();
      filterAndRenderTable();
      
      // Close Case Details Modal if deleting the active folder
      if (activeCaseRef === caseRef) {
        closeCaseDetail();
      }
    } catch (err) {
      console.error("Deletion failure:", err);
      showToast("Error deleting case: " + err.message);
    }
  }
  ...
  deleteCaseBtn.addEventListener('click', () => {
    if (activeCaseRef) executeDeleteCase(activeCaseRef);
  });
